    ...
    window.addEventListener('error', (event) => {
      console.error('Platform JS error:', event.error || event.message);
      if (typeof toast === 'function') toast('Ошибка интерфейса: ' + (event.message || 'проверьте консоль'), 'error');
    });
    window.addEventListener('unhandledrejection', (event) => {
      console.error('Platform async error:', event.reason);
      const msg = event.reason?.message || String(event.reason || 'неизвестная ошибка');
      if (typeof toast === 'function') toast('Ошибка загрузки данных: ' + msg, 'error');
    });
    ...
    async function safeQuery(label, builder, fallback = []) {
      try {
        ...
        toast('Supabase: ' + label + ' недоступно', 'error');
    ...
    async function loadStudentData() {
      ...
      const [materials, homework, slots, requests, packages, payments, lessonLogs, quizzes, quizQuestions, quizAttempts] = await Promise.all([
        supabaseClient.from('materials').select('*, topics(title)').or(`student_id.is.null,student_id.eq.${state.studentProfile.id}`).order('created_at', { ascending:false }),
        supabaseClient.from('homework').select('*, topics(title), lesson_types(title, price, duration)').eq('student_id', state.studentProfile.id).order('deadline', { ascending:true }),
        supabaseClient.from('slots').select('*, lesson_types(title, subject, price, duration)').eq('student_profile_id', state.studentProfile.id).order('date').order('time'),
        supabaseClient.from('lesson_requests').select('*').eq('student_id', state.studentProfile.id).order('created_at', { ascending:false }),
        supabaseClient.from('student_packages').select('*, lesson_types(title, price, duration), subscriptions(title, lessons_count, discount_percent)').eq('student_id', state.studentProfile.id).order('created_at', { ascending:false }),
        supabaseClient.from('payments').select('*, lesson_types(title), student_packages(lessons_total, lessons_used)').eq('student_id', state.studentProfile.id).order('created_at', { ascending:false }),
        supabaseClient.from('lesson_logs').select('*, slots(date,time), lesson_types(title)').eq('student_id', state.studentProfile.id).order('created_at', { ascending:false }),
        supabaseClient.from('quizzes').select('*, topics(title)').or(`student_id.is.null,student_id.eq.${state.studentProfile.id}`).eq('is_active', true).order('created_at', { ascending:false }),
        supabaseClient.from('quiz_questions').select('*').order('created_at'),
        supabaseClient.from('quiz_attempts').select('*, quizzes(title)').eq('student_id', state.studentProfile.id).order('created_at', { ascending:false })
      ]);
...
/* ===== CLOUD FUNCTIONAL TEACHER SECTIONS PATCH v4: stable CRM + safe Supabase loading ===== */
(function(){
  if (window.__cloudTeacherPatchReady) return;
  window.__cloudTeacherPatchReady = true;
  ...
  async function ensureTeacherData() {
    if (typeof window.loadTeacherData === 'function') {
      try { await window.loadTeacherData(); return; } catch(e) { console.warn(e); }
    }
    if (typeof window.loadTeacherCabinet === 'function') {
      try { await window.loadTeacherCabinet(); } catch(e) { console.warn(e); }
    }
  }

  async function createGoogleSyncEvent(type, payload) {
    if (!window.supabaseClient) return;
    try {
      await supabaseClient.from('google_sync_events').insert({
        type,
        status: 'pending',
        payload: payload || {}
      });
    } catch (e) {
      // fallback to older queue
      await supabaseClient.from('notification_events').insert({
        type: 'google_sync_' + type,
        channel: 'google',
        status: 'pending',
        payload: payload || {}
      });
  ...
  function teacherMount(page) {
    return $id(page + 'Mount');
  }
  ...
  window.renderTeacherStudentsFunctional = async function() {
    ...
    mount.innerHTML = `
      <div class="teacher-tool-grid">
        ...
        <div class="panel">
          <h3>Ученики в облаке <span class="cloud-sync-pill">Supabase</span></h3>
          <div class="item-list">
            ${students.map(s => `
              <article class="learning-item">
      ...
      <div class="teacher-tool-grid">
        ...
        <div class="panel">
          ...
          <form class="form-grid" id="cloudAddMaterialForm">
            <div class="field"><label>Название</label><input id="cloudMaterialTitle" required></div>
            <div class="field"><label>Тема</label><select id="cloudMaterialTopic">${topics.map(t=>`<option value="${t.id}">${esc2(t.title)}</option>`).join('')}</select></div>
            ...
            <div class="field"><label>Ссылка</label><input id="cloudMaterialUrl" placeholder="https://..."></div>
        ...
        <div class="panel teacher-tool-full">
          ...
          <div class="item-list">${materials.map(m=>`
            <article class="learning-item">
              <strong>${esc2(m.title)}</strong>
              <p class="muted">${esc2(m.topics?.title || '')}</p>
              <p>${esc2(m.description || '')}</p>
              ${m.file_url || m.url ? `<a class="btn small soft" target="_blank" href="${esc2(m.file_url || m.url)}">Открыть</a>` : ''}
  ...
  window.renderTeacherHomeworkFunctional = async function(reviewOnly=false) {
    ...
    mount.innerHTML = `
      <div class="teacher-tool-grid">
        ...
        <div class="panel">
          <h3>Выдать ДЗ</h3>
          <form class="form-grid" id="cloudAddHomeworkForm">
        ...
        <div class="panel ${reviewOnly ? 'teacher-tool-full' : ''}">
          <h3>${reviewOnly ? 'Присланные работы' : 'Домашние задания'}</h3>
          <div class="item-list">${filtered.map(h=>`
            <article class="learning-item">
              <div class="top"><strong>${esc2(h.title)}</strong><span class="status ${h.is_done ? 'done' : 'pending'}">${h.grade_percent != null ? 'проверено' : (h.is_done ? 'сдано' : 'в работе')}</span></div>
              <p class="muted">${esc2(h.student_profiles?.name || '')} · ${esc2(h.deadline || 'без срока')}</p>
              ...
              <div class="row-actions">
                <button class="btn small" data-cloud-grade-homework="${h.id}">Оценить</button>
                <button class="btn small soft" data-cloud-remind-homework="${h.id}">Напомнить</button>
              </div>
            </article>`).join('') || '<div class="empty">Пока пусто</div>'}</div>
        </div>
      </div>`;
  };
  ...
  window.renderTeacherBookingsFunctional = async function() {
    ...
    mount.innerHTML = `
      <div class="panel">
        <h3>Заявки на занятия <span class="cloud-sync-pill">Supabase</span></h3>
        <div class="item-list">
          ${requests.map(r=>`
            <article class="learning-item">
              <div class="top"><strong>${esc2(r.name || r.student_profiles?.name || 'Заявка')}</strong><span class="status ${esc2(r.status)}">${typeof humanStatus === 'function' ? humanStatus(r.status) : r.status}</span></div>
              <p class="muted">${esc2(r.contact || r.email || '')}</p>
              <p>${esc2(r.direction || '')} ${r.format ? '· ' + esc2(r.format) : ''}</p>
              <p class="muted">${r.slots?.date ? esc2(r.slots.date + ' ' + r.slots.time) : 'Время согласуем'}</p>
              <div class="row-actions">
                <button class="btn small green" data-cloud-approve-booking="${r.id}">Подтвердить</button>
                <button class="btn small red" data-cloud-reject-booking="${r.id}">Отклонить</button>
              </div>
            </article>`).join('') || '<div class="empty">Заявок пока нет</div>'}
        </div>
      </div>`;
  };
  ...
  window.renderTeacherScheduleFunctional = async function() {
    ...
    times.forEach(time => {
      ...
      days.forEach(day => {
        ...
        grid.push(`<div class="slot-editor-cell" data-cloud-add-slot-date="${date}" data-cloud-add-slot-time="${time}">
          ${cellSlots.map(s=>`
            <div class="slot-editor-chip ${esc2(s.status)}">
              <span>${esc2(s.lesson_types?.title || 'Слот')}</span>
              <span>${esc2(typeof humanStatus === 'function' ? humanStatus(s.status) : s.status)} · ${fmtMoney(s.price || s.lesson_types?.price)}</span>
              <div class="row-actions" style="margin-top:4px">
                <button class="btn small green" data-cloud-slot-complete="${s.id}">✓</button>
                <button class="btn small soft" data-cloud-slot-edit="${s.id}">↔</button>
                <button class="btn small red" data-cloud-slot-delete="${s.id}">×</button>
              </div>
            </div>`).join('') || '<span class="calendar-add-hint">+ добавить слот</span>'}
        </div>`);
      });
    });
    ...
    mount.innerHTML = `
      <div class="panel">
        <div class="top-actions">
          <h3>Календарь слотов <span class="cloud-sync-pill">Supabase + Google Calendar queue</span></h3>
          <div class="row-actions" style="margin-top:0">
            <button class="btn small soft" id="cloudSlotPrev">← Неделя</button>
            <button class="btn small soft" id="cloudSlotToday">Сегодня</button>
            <button class="btn small soft" id="cloudSlotNext">Неделя →</button>
          </div>
        </div>
        <div class="notice" style="margin-bottom:12px">Клик по пустой ячейке добавляет слот в Supabase. Кнопки внутри слота удаляют, переносят или отмечают занятие проведённым. Для Google Calendar создаётся очередь синхронизации.</div>
        <div class="slot-editor-calendar">${grid.join('')}</div>
      </div>
      <div class="panel">
        <h3>Быстрое создание слота</h3>
        <form class="form-grid" id="cloudManualSlotForm">
          <div class="grid-2">
            <div class="field"><label>Дата</label><input id="cloudSlotDate" type="date" value="${todayIso()}" required></div>
            <div class="field"><label>Время</label><input id="cloudSlotTime" type="time" value="16:00" required></div>
          </div>
          <div class="field"><label>Тип занятия</label><select id="cloudSlotLessonType">${lessonTypes.map(lt=>`<option value="${lt.id}" data-duration="${lt.duration || 60}" data-price="${lt.price || 2000}">${esc2(lt.title)} · ${fmtMoney(lt.price)}</option>`).join('')}</select></div>
          <div class="grid-2">
            <div class="field"><label>Длительность</label><input id="cloudSlotDuration" type="number" value="60"></div>
            <div class="field"><label>Цена</label><input id="cloudSlotPrice" type="number" value="2000"></div>
          </div>
          <button class="btn" type="submit">Добавить слот в облако</button>
        </form>
      </div>`;
  };
  ...
  window.renderTeacherFinanceFunctional = async function(paymentsOnly=false) {
    ...
    mount.innerHTML = `
      <div class="teacher-tool-grid">
        ...
        <div class="panel">
          <h3>Тип занятия</h3>
          <form class="form-grid" id="cloudAddLessonTypeForm">
            <div class="field"><label>Название</label><input id="cloudLessonTypeTitle" required></div>
            <div class="field"><label>Предмет</label><input id="cloudLessonTypeSubject" placeholder="Математика"></div>
            <div class="grid-2"><div class="field"><label>Длительность</label><input id="cloudLessonTypeDuration" type="number" value="60"></div><div class="field"><label>Цена</label><input id="cloudLessonTypePrice" type="number" value="2000"></div></div>
            <button class="btn" type="submit">Сохранить тип</button>
          </form>
        </div>
        <div class="panel">
          <h3>Абонементы</h3>
          <div class="item-list">${subscriptions.map(s=>`<article class="learning-item"><strong>${esc2(s.title)}</strong><p>${s.lessons_count || 0} занятий · скидка ${s.discount_percent || 0}%</p></article>`).join('') || '<div class="empty">Абонементов пока нет</div>'}</div>
        </div>`}
        <div class="panel teacher-tool-full">
          <h3>Добавить оплату</h3>
          <form class="form-grid" id="cloudAddPaymentForm">
            <div class="field"><label>Ученик</label><select id="cloudPaymentStudent"><option value="">Без ученика</option>${students.map(s=>`<option value="${s.id}">${esc2(s.name)}</option>`).join('')}</select></div>
            <div class="grid-2"><div class="field"><label>Сумма</label><input id="cloudPaymentAmount" type="number" value="2000"></div><div class="field"><label>Статус</label><select id="cloudPaymentStatus"><option value="paid">Оплачено</option><option value="pending">Ожидает</option><option value="cancelled">Отменено</option></select></div></div>
            <div class="field"><label>Комментарий</label><input id="cloudPaymentComment"></div>
            <button class="btn" type="submit">Сохранить оплату</button>
          </form>
        </div>
        <div class="panel teacher-tool-full">
          <h3>Оплаты</h3>
          <div class="item-list">${payments.map(p=>`<article class="learning-item"><div class="top"><strong>${fmtMoney(p.amount)}</strong><span class="status ${esc2(p.status)}">${esc2(typeof humanStatus === 'function' ? humanStatus(p.status) : p.status)}</span></div><p class="muted">${esc2(p.student_profiles?.name || '')}</p><p>${esc2(p.notes || '')}</p></article>`).join('') || '<div class="empty">Оплат пока нет</div>'}</div>
        </div>
      </div>`;
  };
    ...
    mount.innerHTML = `
      <div class="teacher-tool-grid">
        <div class="panel">
          ...
          <form class="form-grid" id="cloudAddReviewForm">
            <div class="field"><label>Автор</label><input id="cloudReviewAuthor" required></div>
            <div class="field"><label>Оценка</label><input id="cloudReviewRating" type="number" min="1" max="5" value="5"></div>
            <div class="field"><label>Текст</label><textarea id="cloudReviewText" required></textarea></div>
            <div class="field"><label>Подтверждающая ссылка</label><input id="cloudReviewProofUrl"></div>
            <button class="btn" type="submit">Сохранить отзыв</button>
          </form>
        </div>
        <div class="panel">
          <h3>Отзывы</h3>
          <div class="item-list">${reviews.map(r=>`<article class="learning-item"><strong>${esc2(r.author)}</strong><p>${esc2(r.text)}</p></article>`).join('') || '<div class="empty">Отзывов пока нет</div>'}</div>
        </div>
        <div class="panel">
          <h3>Кейсы</h3>
          <div class="item-list">${cases.map(c=>`<article class="learning-item"><strong>${esc2(c.title)}</strong><p>${esc2(c.result_text || '')}</p></article>`).join('') || '<div class="empty">Кейсов пока нет</div>'}</div>
        </div>
        <div class="panel">
          <h3>Примеры материалов</h3>
          <div class="item-list">${examples.map(x=>`<article class="learning-item"><strong>${esc2(x.title)}</strong><p>${esc2(x.description || '')}</p></article>`).join('') || '<div class="empty">Примеров пока нет</div>'}</div>
        </div>
      </div>`;
  };
  ...
  // universal click fallback: data-page, buttons inside dynamic panels, slot actions.
  document.addEventListener('click', async function(e){
    const pageBtn = e.target.closest('[data-page]');
    if (pageBtn && typeof window.setPage === 'function') {
      e.preventDefault();
      window.setPage(pageBtn.dataset.page);
      return;
    }
    ...
    if (editStudent) {
      ...
      if (!s) return;
      const name = prompt('Имя ученика', s.name || '');
      if (name === null) return;
      const subject = prompt('Предмет / курс', s.subject || '') ?? (s.subject || '');
      const telegram = prompt('Telegram', s.telegram || '') ?? (s.telegram || '');
      const { error } = await supabaseClient.from('student_profiles').update({ name, subject, telegram }).eq('id', id);
      if (error) return toast2(error.message, 'error');
      await createGoogleSyncEvent('sheet_student_updated', { id, name, subject, telegram });
      toast2('Карточка ученика обновлена.');
      await renderTeacherStudentsFunctional();
      return;
    }
    ...
    if (deleteStudent) {
      e.preventDefault();
      if (!confirm('Удалить ученика? Связанные ДЗ/материалы могут быть удалены настройками базы.')) return;
      const id = deleteStudent.dataset.cloudDeleteStudent;
      const { error } = await supabaseClient.from('student_profiles').delete().eq('id', id);
      if (error) return toast2(error.message, 'error');
      await createGoogleSyncEvent('sheet_student_deleted', { id });
      toast2('Ученик удалён.');
      await renderTeacherStudentsFunctional();
      return;
    }

        const addSlot = e.target.closest('[data-cloud-add-slot-date]');
    if (addSlot && !e.target.closest('button')) {
      e.preventDefault();
      const date = addSlot.dataset.cloudAddSlotDate;
      const time = addSlot.dataset.cloudAddSlotTime;
      const title = prompt('Название / тип слота', 'Занятие');
      ...
      const { error, data } = await supabaseClient.from('slots').insert(row).select('*').single();
      if (error) return toast2(error.message, 'error');
      await createGoogleSyncEvent('calendar_slot_created', { slot: data || row });
      toast2('Слот добавлен в облако.');
      await renderTeacherScheduleFunctional();
      if (typeof loadPublicSlots === 'function') loadPublicSlots();
      return;
    }

    const del = e.target.closest('[data-cloud-slot-delete]');
    if (del) {
      e.preventDefault(); e.stopPropagation();
      if (!confirm('Удалить слот?')) return;
      const id = del.dataset.cloudSlotDelete;
      const { error } = await supabaseClient.from('slots').delete().eq('id', id);
      if (error) return toast2(error.message, 'error');
      await createGoogleSyncEvent('calendar_slot_deleted', { slot_id:id });
      toast2('Слот удалён.');
      await renderTeacherScheduleFunctional();
      if (typeof loadPublicSlots === 'function') loadPublicSlots();
      return;
    ...
    if (complete) {
      e.preventDefault(); e.stopPropagation();
      const id = complete.dataset.cloudSlotComplete;
      const { error } = await supabaseClient.from('slots').update({ status:'completed' }).eq('id', id);
      if (error) return toast2(error.message, 'error');
      await createGoogleSyncEvent('calendar_slot_completed', { slot_id:id });
      toast2('Занятие отмечено проведённым.');
      await renderTeacherScheduleFunctional();
      return;
    }

    const edit = e.target.closest('[data-cloud-slot-edit]');
    if (edit) {
      e.preventDefault(); e.stopPropagation();
      const id = edit.dataset.cloudSlotEdit;
      const date = prompt('Новая дата YYYY-MM-DD');
      if (!date) return;
      const time = prompt('Новое время HH:MM', '16:00');
      if (!time) return;
      const { error } = await supabaseClient.from('slots').update({ date, time, status:'rescheduled' }).eq('id', id);
      if (error) return toast2(error.message, 'error');
      await createGoogleSyncEvent('calendar_slot_updated', { slot_id:id, date, time });
      toast2('Слот перенесён.');
      await renderTeacherScheduleFunctional();
      if (typeof loadPublicSlots === 'function') loadPublicSlots();
      return;
    }
    ...
    if (grade) {
      e.preventDefault();
      if (typeof window.gradeHomework === 'function') return window.gradeHomework(grade.dataset.cloudGradeHomework);
      return;
    }
    ...
    if (approve) {
      e.preventDefault();
      if (typeof window.approveBookingRequest === 'function') await window.approveBookingRequest(approve.dataset.cloudApproveBooking);
      await renderTeacherBookingsFunctional();
      return;
    }
    ...
    if (reject) {
      e.preventDefault();
      if (typeof window.rejectBookingRequest === 'function') await window.rejectBookingRequest(reject.dataset.cloudRejectBooking);
      await renderTeacherBookingsFunctional();
      return;
    }
  }, true);
  ...
  document.addEventListener('submit', async function(e){
    const f = e.target;
    if (f.id === 'cloudAddStudentForm') {
      e.preventDefault();
      const row = { name:$id('cloudStudentName').value.trim(), email:$id('cloudStudentEmail').value.trim(), subject:$id('cloudStudentCourse').value.trim(), telegram:$id('cloudStudentTelegram').value.trim() };
      const { error } = await supabaseClient.from('student_profiles').insert(row);
      if (error) return toast2(error.message,'error');
      await createGoogleSyncEvent('sheet_student_created', row);
      toast2('Ученик сохранён в облаке.');
      await renderTeacherStudentsFunctional();
      return;
    }

    if (f.id === 'cloudAddTopicForm') {
      e.preventDefault();
      const row = { title:$id('cloudTopicTitle').value.trim(), subject:$id('cloudTopicSubject').value.trim(), description:$id('cloudTopicDescription').value.trim() };
      const { error } = await supabaseClient.from('topics').insert(row);
      if (error) return toast2(error.message,'error');
      await createGoogleSyncEvent('sheet_topic_created', row);
      toast2('Тема сохранена.');
      await renderTeacherMaterialsFunctional();
      return;
    }

    if (f.id === 'cloudAddMaterialForm') {
      e.preventDefault();
      const row = { title:$id('cloudMaterialTitle').value.trim(), topic_id:$id('cloudMaterialTopic').value || null, student_id:$id('cloudMaterialStudent').value || null, file_url:$id('cloudMaterialUrl').value.trim(), description:$id('cloudMaterialDescription').value.trim() };
      const { error } = await supabaseClient.from('materials').insert(row);
      if (error) return toast2(error.message,'error');
      await createGoogleSyncEvent('drive_material_created', row);
      toast2('Материал сохранён.');
      await renderTeacherMaterialsFunctional();
      return;
    }

    if (f.id === 'cloudAddHomeworkForm') {
      e.preventDefault();
      const row = { student_id:$id('cloudHomeworkStudent').value, topic_id:$id('cloudHomeworkTopic').value || null, title:$id('cloudHomeworkTitle').value.trim(), description:$id('cloudHomeworkDescription').value.trim(), deadline:$id('cloudHomeworkDeadline').value || null, is_done:false };
      const { error, data } = await supabaseClient.from('homework').insert(row).select('*').single();
      if (error) return toast2(error.message,'error');
      await createGoogleSyncEvent('sheet_homework_created', { homework:data || row });
      await supabaseClient.from('notification_events').insert({ type:'homework_created', channel:'email', status:'pending', payload: row });
      toast2('ДЗ выдано и сохранено в облаке.');
      await renderTeacherHomeworkFunctional(false);
      return;
    }

    if (f.id === 'cloudManualSlotForm') {
      e.preventDefault();
      const row = { date:$id('cloudSlotDate').value, time:$id('cloudSlotTime').value, lesson_type_id:$id('cloudSlotLessonType').value || null, duration:Number($id('cloudSlotDuration').value || 60), price:Number($id('cloudSlotPrice').value || 2000), status:'open', currency:'RUB' };
      const { error, data } = await supabaseClient.from('slots').insert(row).select('*').single();
      if (error) return toast2(error.message,'error');
      await createGoogleSyncEvent('calendar_slot_created', { slot:data || row });
      toast2('Слот добавлен в облако.');
    ...
    if (f.id === 'cloudAddPaymentForm') {
      e.preventDefault();
      const row = { student_id:$id('cloudPaymentStudent').value || null, amount:Number($id('cloudPaymentAmount').value || 0), status:$id('cloudPaymentStatus').value, notes:$id('cloudPaymentComment').value.trim(), currency:'RUB' };
      const { error, data } = await supabaseClient.from('payments').insert(row).select('*').single();
      if (error) return toast2(error.message,'error');
      await createGoogleSyncEvent('sheet_payment_created', { payment:data || row });
      toast2('Оплата сохранена.');
      await renderTeacherFinanceFunctional(true);
      return;
    }

    if (f.id === 'cloudAddLessonTypeForm') {
      e.preventDefault();
      const row = { title:$id('cloudLessonTypeTitle').value.trim(), subject:$id('cloudLessonTypeSubject').value.trim(), duration:Number($id('cloudLessonTypeDuration').value || 60), price:Number($id('cloudLessonTypePrice').value || 2000), currency:'RUB', format:'online', is_active:true };
      const { error } = await supabaseClient.from('lesson_types').insert(row);
      if (error) return toast2(error.message,'error');
      await createGoogleSyncEvent('sheet_lesson_type_created', row);
      toast2('Тип занятия сохранён.');
      await renderTeacherFinanceFunctional(false);
      return;
    }

    if (f.id === 'cloudAddReviewForm') {
      e.preventDefault();
      const row = { author:$id('cloudReviewAuthor').value.trim(), rating:Number($id('cloudReviewRating').value || 5), text:$id('cloudReviewText').value.trim(), proof_url:$id('cloudReviewProofUrl').value.trim(), is_published:true };
      const { error } = await supabaseClient.from('reviews').insert(row);
      if (error) return toast2(error.message,'error');
      await createGoogleSyncEvent('sheet_review_created', row);
      toast2('Отзыв сохранён.');
      await renderTeacherContentFunctional();
      return;
    }
  }, true);
  ...
  if (typeof oldSetPage === 'function') {
    window.setPage = function(page) {
      oldSetPage(page);
      setTimeout(() => renderTeacherPage(page), 120);
    };
  }
})();
