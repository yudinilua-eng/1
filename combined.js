    ...
    async function loadTeacherData() {
      ...
      state.homework = (homework.data || []).map(h => ({
        ...h,
        student_profiles: state.students.find(s => s.id === h.student_id) || h.student_profiles || null,
        topics: state.topics.find(t => t.id === h.topic_id) || h.topics || null,
        lesson_types: (lessonTypes.data || []).find(t => t.id === h.lesson_type_id) || h.lesson_types || null
      }));
    ...
    window.toggleLessonType = async function(id, isActive) {
      ...
      if (status === 'completed') {
        await autoDeductPackageForSlot(id);
      }
      ...
      if (status === 'confirmed' || status === 'cancelled' || status === 'completed') {
        await supabaseClient.from('notification_events').insert({
          ...
          channel: 'email',
          status: 'pending',
          payload: { to: state.siteSettings?.notification_email || 'bkmz.lby@mail.ru', slotId: id, status }
        });
      }
      await loadTeacherCabinet();
    }
...
/* ===== CLOUD FUNCTIONAL TEACHER SECTIONS PATCH ===== */
(function(){
  ...
  document.addEventListener('submit', async function(e){
    ...
    if (f.id === 'cloudAddMaterialForm') {
      e.preventDefault();
      const row = { title:$id('cloudMaterialTitle').value.trim(), topic_id:$id('cloudMaterialTopic').value || null, student_id:$id('cloudMaterialStudent').value || null, url:$id('cloudMaterialUrl').value.trim(), description:$id('cloudMaterialDescription').value.trim() };
