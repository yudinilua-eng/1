...
(function(){
  if (window.__platformV24CabinetReady) return;
  window.__platformV24CabinetReady = true;
  const labels = {
    studentCabinet:'Личный кабинет ученика',
    teacherCabinet:'Обзор кабинета',
    teacherStudents:'Ученики',
    teacherMaterials:'Материалы и темы',
    teacherHomework:'Домашние задания',
    teacherHomeworkReview:'Проверка ДЗ',
    teacherSchedule:'Расписание и слоты',
    teacherBookings:'Заявки на занятия',
    teacherNotifications:'Уведомления',
    teacherFinance:'Финансы',
    teacherPayments:'Оплаты',
    teacherContent:'Отзывы и кейсы',
    analytics:'Аналитика',
    activityLog:'История действий',
    settings:'Настройки сайта',
    integrations:'Материалы Яндекс',
    today:'Сегодня'
  };
  const cabinetPages = new Set(Object.keys(labels));
  function activePage(){
    const active = document.querySelector('.page.active');
    return active ? active.id.replace(/^page-/,'') : 'home';
  }
  function isStudent(){ return document.body.classList.contains('is-student') || !!localStorage.getItem('student_session_id'); }
  function isLogged(){ return document.body.classList.contains('is-logged') || isStudent() || !!(window.state && window.state.user); }
  async function logout(){
    ...
    if (window.state) { window.state.user=null; window.state.profile=null; window.state.studentProfile=null; }
    document.body.classList.remove('is-logged','is-admin','is-student','is-parent');
    if (typeof window.toast === 'function') window.toast('Вы вышли из профиля','success');
    if (typeof window.setPage === 'function') window.setPage('home');
  }
  function ensureCabinetHeader(){
    const container = document.querySelector('.content > .container');
    if (!container) return null;
    let header = document.getElementById('cabinetHeaderV24');
    if (!header) {
      header = document.createElement('div');
      header.id = 'cabinetHeaderV24';
      header.innerHTML = '<div class="cabinet-header-title"><small id="cabinetSubtitleV24">Рабочая зона платформы</small><h1 id="cabinetTitleV24">Личный кабинет</h1></div><div class="cabinet-header-actions"><button class="btn small soft" type="button" id="cabinetProfileBtnV24"><i class="fa-solid fa-user"></i> Профиль</button><button class="btn small red" type="button" id="cabinetLogoutBtnV24"><i class="fa-solid fa-right-from-bracket"></i> Выйти</button></div>';
      container.prepend(header);
      document.getElementById('cabinetProfileBtnV24')?.addEventListener('click', ()=>{
        if (typeof window.setPage === 'function') window.setPage(isStudent() ? 'studentCabinet' : 'teacherCabinet');
      });
      document.getElementById('cabinetLogoutBtnV24')?.addEventListener('click', logout);
    }
    return header;
  }
  function cleanupCabinet(){
    const page = activePage();
    const cabinet = cabinetPages.has(page);
    document.body.classList.toggle('cabinet-route', cabinet);
    document.body.classList.toggle('public-route', !cabinet);
    if (!cabinet) {
      const h = document.getElementById('cabinetHeaderV24');
      if (h) h.style.display = 'none';
      return;
    }
    const header = ensureCabinetHeader();
    if (header) header.style.display = 'flex';
    const title = document.getElementById('cabinetTitleV24');
    const sub = document.getElementById('cabinetSubtitleV24');
    if (title) title.textContent = labels[page] || 'Личный кабинет';
    if (sub) sub.textContent = isStudent() ? 'Кабинет ученика · материалы, задания и прогресс' : 'Кабинет преподавателя · ученики, заявки, расписание и материалы';
    document.querySelectorAll('#profileQuickbar,#profileDockV17').forEach(el=>el.remove());
    document.querySelectorAll('aside.sidebar .sidebar-login-card, aside.sidebar .top-login-cabinet-btn').forEach(el=>el.remove());
    document.querySelectorAll('aside.sidebar .nav-btn[data-page]').forEach(btn=>{
      btn.classList.toggle('active', btn.dataset.page === page);
      if (btn.dataset.page === page) btn.setAttribute('aria-current','page'); else btn.removeAttribute('aria-current');
      if (btn.tagName === 'BUTTON') {
        const pathMap = window.pageToPath || {};
        if (pathMap[btn.dataset.page] && !btn.dataset.v24LinkWrapped) {
          btn.dataset.v24LinkWrapped = '1';
        }
      }
    });
    document.querySelectorAll('aside.sidebar .menu-group').forEach(group=>{
      const hasActive = !!group.querySelector('.nav-btn.active');
      if (hasActive) group.setAttribute('open','');
      group.classList.toggle('active-branch', hasActive);
    });
    document.querySelectorAll('.page.active input,.page.active textarea,.page.active select').forEach(el=>{
      if (!el.hasAttribute('autocomplete') && (el.type === 'password')) el.setAttribute('autocomplete','new-password');
    });
    document.querySelectorAll('.page.active .panel:empty,.page.active .soft-section:empty,.page.active .mini-panel:empty').forEach(el=>el.remove());
  }
  const oldSetPage = window.setPage;
  if (typeof oldSetPage === 'function' && !window.__platformV24WrappedSetPage) {
    window.__platformV24WrappedSetPage = true;
    window.setPage = function(){
      const result = oldSetPage.apply(this, arguments);
      requestAnimationFrame(cleanupCabinet);
      setTimeout(cleanupCabinet, 80);
      setTimeout(cleanupCabinet, 320);
      return result;
    };
  }
  document.addEventListener('DOMContentLoaded', cleanupCabinet);
  window.addEventListener('load', cleanupCabinet);
  document.addEventListener('click', e=>{
    const nav = e.target.closest('aside.sidebar .nav-btn[data-page]');
    if (!nav) return;
    const page = nav.dataset.page;
    const path = (window.pageToPath && window.pageToPath[page]) || null;
    if (path && (e.metaKey || e.ctrlKey || e.button === 1)) {
      e.preventDefault();
      window.open(path, '_blank');
    }
  }, true);
  setTimeout(cleanupCabinet, 150);
  setTimeout(cleanupCabinet, 900);
})();
